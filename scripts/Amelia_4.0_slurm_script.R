...
colnames(trait.data) <- c("tips", "Diel_Pattern")
...
if("bridge_only" %in% args & "four_state_max_crep" %in% args){
  bridge_only <- lapply(phylo_trees, function(x) returnCorModels(tree = x, trait.data = trait.data, diel_col = "Diel_Pattern", rate.cat = 1, custom.rate.mat = matrix(c(0,1,2,3,4,0,5,6,7,8,0,0,10,11,0,0), ncol = 4, nrow = 4), model = "ARD", node.states = "marginal"))
}

#try running a constrained bridge only model based on a symmetrical model
if("CONSYM" %in% args & "four_state_max_crep" %in% args){
  CONSYM <- lapply(phylo_trees, function(x) returnCorModels(tree = x, trait.data = trait.data, diel_col = "Diel_Pattern", rate.cat = 1, custom.rate.mat = matrix(c(0,1,2,3,1,0,5,6,2,5,0,0,3,6,0,0), ncol = 4, nrow = 4), model = "SYM", node.states = "marginal"))
}

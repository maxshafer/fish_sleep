...
for (y in 1:length(model_types)) {
  ...
  for (i in 1:length(index_list)) {
    ...
    for (j in 1:length(index_list[[i]])) {
      ...
      plot <- simulatedSwitchRatio(simulated_cumsums = cumsums, phylo_tree = trpy_n, node.age.cutoff = 0.02, plot_type = "summary", highlight_colour = "red", error = "sd") #+ ylim(c(0,0.15))
      ...
      ## Save out the plot
      pdf(file = here(paste("outs/Figures/plot_15_simulations_states", dataset_variable, name_variable, length(trpy_n$tip.label), "species", model_type, ncol(simulation), "zoom.pdf", sep = "_")), width = 10, height = 10)
      print(recon_plot / geo_scale + plot_layout(nrow = 2, heights = c(5,0.5)))
      dev.off()
      
      if (model_type == "HR") {
        plot_simulation_rates <- simulatedSwitchRatio(simulated_cumsums = cumsums_rates, phylo_tree = trpy_n, node.age.cutoff = 0.02, plot_type = "summary", highlight_colour = "red", error = "sd") #+ ylim(c(0,0.15))
        ...
        pdf(file = here(paste("outs/Figures/plot_16_simulations_rates", dataset_variable, name_variable, length(trpy_n$tip.label), "species", model_type, ncol(simulation), "zoom.pdf", sep = "_")), width = 10, height = 10)

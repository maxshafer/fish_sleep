...
trait.data[trait.data$Family == "Ziphiidae", c("fam_colours")] <- "green1"
...
trait.data[trait.data$Family == "Eschrichtiidae", c("fam_colours")] <- "yellow4"
...
ggplot(trait.data, aes(x = max_crep, y = log(Body_mass))) + geom_boxplot(outlier.shape = NA) + 
  geom_jitter(aes(fill = Family), size = 3, width = 0.1, height = 0, colour = "black", pch = 21) + 
  scale_fill_manual(values = unique(trait.data$fam_colours)) + facet_wrap(~Parvorder)

ggplot(trait.data, aes(x = max_crep, y = Dive_depth)) + geom_boxplot(outlier.shape = NA) + 
  geom_jitter(aes(fill = Family), size = 3, width = 0.1, height = 0, colour = "black", pch = 21) + 
  scale_fill_manual(values = unique(trait.data$fam_colours))# + facet_wrap(~Parvorder)

...
#example species
#x <- filter(test_diel_long, Species_name == "Cephalorhynchus commersonii")

# my method: take mode of mutliple level 4 sources if they exist, if not mode of Conf3 and Conf4
#if unclear add in Conf5 data, if unclear use single level 4 source (if it exists), if still unclear call cathemeral-variable or include Conf1 and Conf2 data
  ...
  if(x %>% filter(column == "Conf4") %>% nrow() > 1){
    ...
    if (is.na(which.max.simple(tabulate(match(x$new_diel[x$column %in% c("Conf5", "Conf4", "Conf3")], unique(x$new_diel))), tie_value = "NA"))) {
      if (is.na(which.max.simple(tabulate(match(x$new_diel[x$column %in% c("Conf5", "Conf4", "Conf3", "Conf2", "Conf1")], unique(x$new_diel))), tie_value = "NA"))) {
        ...
        #result if there is not a tie (use the single level 4 entry to make the call)
        activity_pattern <- unique(x$new_diel)[which.max.simple(tabulate(match(x$new_diel[x$column %in% c("Conf4")], unique(x$new_diel))), tie_value = "NA")]
        #if there is no level 4 source it will return an NA and move on to here
        if(NA %in% activity_pattern){
          #check if there is only level 1 and 2 evidence, if so use them to make the call but flag as low confidence
          if(nrow(x[x$column %in% c("Conf1", "Conf2"),]) == nrow(x)){
            ...
            activity_pattern <- paste(activity_pattern, "low-confidence", sep = "-")
          } else {
            #this is the case where there are mulitple level 3 or 5 sources that are tied, so we can call it cathemeral-variable
            activity_pattern <- "cathemeral-variable"
          }
        }
         } 
...
x <- filter(test_diel_long, Species_name == "Balaena mysticetus")
x <- filter(test_diel_long, Species_name == "Lipotes vexillifer")
x <- filter(test_diel_long, Species_name == "Balaenoptera acutorostrata")

#probably want to create a better method
#if category 3 greater than 50%
#if category 4 higher than 0%
#if category 2 higher than 75%
...
tabulateCrep = function(x){
  ...
  #confidence level 1 and 5 don't have any information on crepuscularity so remove them
  ...
  df3 <- merge(df, df2, by = "Category")
  df3$percentage <- round((df3$x.x/df3$x.y) * 100, digits = 1)
  #define what percentage of each category we want to call a species crepuscular, FIX THIS
  df3$cutoff <- 20
  #df3$cutoff <- c(0, 75, 50, 0, 0)
  df3$crep_evidence <- df3$percentage > df3$cutoff
  ...
  unique_percents <- unique(df3$crep_evidence)
  total_evidence <- unique_percents[which.max(tabulate(match(df3$crep_evidence, unique_percents)))]
  return(total_evidence)
}
...
diel_full_new$Species_name <- str_replace(diel_full_new$Species_name, pattern = "Physeter catodon", replacement = "Physeter macrocephalus")
diel_full_new$tips <- str_replace(diel_full_new$tips, pattern = "Physeter_catodon", replacement = "Physeter_macrocephalus")
...
custom.colours.2 <- c("#dd8ae7", "peachpuff2", "#FC8D62","#FC8D62", "yellow", "#66C2A5", "#66C2A5","green","black","grey")
...
custom.colours.2 <- c("#dd8ae7", "peachpuff2", "#FC8D62", "#FC8D62","#66C2A5","#66C2A5","grey")
...
diel_full <- merge(diel_full[, c("Species_name", "max_crep")], diel_full_long[c("Species_name", "column", "value")])

diel_full$column <- substr(diel_full$column, 1,5)

#use max crep dataset 
diel_full <- diel_full[diel_full$max_crep %in% c("diurnal", "nocturnal", "cathemeral", "crepuscular"), ]

diel_full$value <- str_replace(diel_full$value, pattern = "diurnal/crepuscular", replacement = "crepuscular")
diel_full$value <- str_replace(diel_full$value, pattern = "nocturnal/crepuscular", replacement = "crepuscular")
diel_full$value <- str_replace(diel_full$value, pattern = "cathemeral/crepuscular", replacement = "crepuscular")
diel_full$value <- str_replace(diel_full$value, pattern = "unclear/crepuscular", replacement = "crepuscular")
diel_full$value <- str_replace(diel_full$value, pattern = "unclear/nocturnal", replacement = "nocturnal")
diel_full$value <- str_replace(diel_full$value, pattern = "unclear/diurnal", replacement = "diurnal")
diel_full$value <- str_replace(diel_full$value, pattern = "unclear/cathemeral", replacement = "cathemeral")
diel_full$value <- str_replace(diel_full$value, pattern = "nocturnal/cathemeral", replacement = "cathemeral")
diel_full$value <- str_replace(diel_full$value, pattern = "diurnal/cathemeral", replacement = "cathemeral")
...
#of these only x are in the final tree
...
ggplot(concordance, aes(actual, predicted, fill = percent)) + geom_tile() + geom_text(aes(label = percent)) +
  scale_fill_gradient(low = "white", high = "dodgerblue") + labs(x = "Actual", y = "Predicted") +
  ggtitle(paste("Confidence level ", substr(set_column, 5,5), " concordance"))

...
boxplot_KW <- ggplot(trait.data, aes(x = Diel_Pattern, y = trait.data[,c(continuous_trait)])) + geom_boxplot(outlier.shape = NA) +  
  ...
  labs(x = "Temporal activity pattern", y = continuous_trait) + scale_fill_manual(values=as.vector(polychrome(26))) +
...
boxplot_anova <- ggplot(trait.data, aes(x = Diel_Pattern, y = trait.data[,c(continuous_trait)])) + geom_boxplot(outlier.shape = NA) + 
  geom_jitter(aes(fill = Family), size = 3, width = 0.1, height = 0, colour = "black", pch = 21) + 
  labs(x = "Temporal activity pattern", y = continuous_trait) + scale_fill_manual(values=trait.data$fam_colours) + theme_minimal() +
  ...
  stat_compare_means(label = "p.format", method = "t.test",ref.group = ".all.") + stat_compare_means(method = "anova") #+ facet_wrap(~Parvorder)
boxplot_anova
...
boxplot_phyloanova <- ggplot(trait.data, aes(x = Diel_Pattern, y = trait.data[,c(continuous_trait)])) + geom_boxplot(outlier.shape = NA) +
  geom_jitter(aes(fill = Family), size = 3, width = 0.1, height = 0, colour = "black", pch = 21) +
  labs(x = "Temporal activity pattern", y = continuous_trait) + scale_fill_manual(values=as.vector(polychrome(26))) +
  annotate("text", x = 1:4, y = 38, label = list("ns", "ns", "ns", "ns")) + annotate("text", x = 1.5, y = 0, label = paste("phylANOVA, p =", phylANOVA$Pf))
...
pdf(paste0("C:/Users/ameli/OneDrive/Documents/R_projects/Amelia_figures/", continuous_trait, "_boxplots_anova_odontocetes.pdf"))
boxplot_anova
dev.off()
...
pdf(paste0("C:/Users/ameli/OneDrive/Documents/R_projects/Amelia_figures/", continuous_trait, "_boxplots_phyloanova_cetaceans.pdf"))
boxplot_phyloanova
dev.off()
